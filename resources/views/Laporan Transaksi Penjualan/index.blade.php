                        ...
                        <tbody>
                            ...
                            @foreach($transaksiPenjualan as $item)
                                <tr>
                                    ...
                                    <td>
                                        <button type="button" class="btn btn-info detail-btn" data-id="{{ $item->no_faktur }}">Detail</button>
...
$(document).ready(function() {
    ...
    $('#pdfButton').on('click', function() {
        ...
        form.attr('action', "{{ route('LaporanTransaksiPenjualan.cetakpdf') }}"); // Ubah action form ke route PDF
        ...
        if (noFaktur == "{{ $transaksi->no_faktur }}") {
            ...
            if (penjualanPerBarang.length > 0) {
                penjualanPerBarang.forEach(item => {
                    ...
                    var listItem = '<tr>' +
                        '<td>' + (item.menu ? item.menu.nama : 'Tidak tersedia') + '</td>' +
                        '<td>' + item.jumlah + '</td>' +
                        '<td>' + item.harga + '</td>' +
                        '<td>' + item.subtotal + '</td>' +
                        '</tr>';
